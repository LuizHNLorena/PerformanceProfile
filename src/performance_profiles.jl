...
function performance_profile(T :: Array{Float64,2}, 
  ...
  xlabel = pop!(kwargs, :xlabel, L"\tau" * (logscale ? "" : ""))            # <<<<<<<<<<<<<<<<<<<<<<<<<<<<
  
  #ylabel = pop!(kwargs, :ylabel, "Proportion of problems")

  ##ylabel = pop!(kwargs, :ylabel, latexstring("ρ_{s}(τ)"))                     # <<<<<<<<<<<<<<<<<<<<<<<<<<<<

  ylabel = pop!(kwargs, :ylabel, L"\rho_{v}(\tau)") #latexstring("\\pho_{2}"))

  linestyles = pop!(kwargs, :linestyles, Symbol[])
  
  profile = Plots.plot(xlabel = xlabel, 
                       ylabel = ylabel, 
                       title = title, 
                       #xlims = (logscale ? 0.0 : 1.0, max_ratio),
                       xlims = (logscale ? 0.0 : 1.0, 1.1 * max_ratio), 
                       ylims = (0, 1.0), 
                       yticks = 0.0:0.1:1.0,
                       legend=:bottomright)  # initial plot
  
  #profile = Plots.plot(xlabel = xlabel, ylabel = ylabel, title = title, xlims = (logscale ? 0.0 : 1.0, 1.1 * max_ratio), ylims = (0, 1.1), legend=:bottomright)  # initial plot
  #####profile = Plots.plot(xlabel = xlabel, ylabel = ylabel, title = title, xlims = (logscale ? 1.0 : 1.0, 1.1 * max_ratio), ylims = (0, 1.1))  # initial plot
  for s = 1 : ns
    rs = view(ratios,:,s)
